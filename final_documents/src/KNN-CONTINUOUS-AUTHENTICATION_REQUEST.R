...
# remove all columns except the 4 we use
...
# normalize the 3 variables we use as predictors
...
# the attack type we are classifying
ATTACKTYPE<-"authentication_request"

# Keep only the target class and the normal packets
wifiLog2<-wifiLog2[wifiLog2$class=="normal" | wifiLog2$class==ATTACKTYPE, ]

# rename the attack type and normal class types to 0/1
...
# The sampling will be 66% training, 33% test
...
library(DMwR)
# the formula of the 3 predictor variables
f<-formula("class~wlan.fc.type+frame.time_delta_displayed+wlan.duration")
# perform the smote synthetic attribute creation
train_smote<-SMOTE(f,train,perc.over=100,perc.under=400,k=1)
View(train_smote)
library(mlr)
