...
# keep only the 4 columns we use
...
# min-max normilize the 3 variables we use for predicting
...
# rename the attacktype and normal type to 0/1
...
library(DMwR)
# formula that contains the 3 variables we use for predicting
f<-formula("class~wlan.fc.type+frame.time_delta_displayed+wlan.duration")
# run smote
train_smote<-SMOTE(f,train,perc.over=120,perc.under=200,k=1)
View(train_smote)
library(mlr)
